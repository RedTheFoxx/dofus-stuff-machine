...
- [x] **Phase 5: Base locale, hors-ligne et resynchronisation** - Le fichier SQLite, la fenêtre de re-check 24 h, les deux défauts hors-ligne et les commandes destructrices sont expliqués sans être exécutés (completed 2026-09-11)
- [x] **Phase 6: Dépannage, glossaire, complétude et preuve finale** - Le lecteur cherche par message d'erreur ou par terme, la complétude est vérifiée, et la suite prouve que les garde-fous mordent (completed 2026-09-11)
...
- [x] 02-01: Rédiger `docs/cli.md` (sous-commandes, options globales, options d'`optimize`, mode interactif, avertissement destructif) et son entrée d'index

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 02-02: Ajouter les contrôles d'ancrage des sous-commandes et des options documentées
...
- [x] 03-03: Ajouter « Sauvegarder et exporter » — limite réelle des sauvegardes du navigateur ancrée sur la constante du JS, éviction silencieuse dite honnêtement, export Dofusbook (seize emplacements, prysmaradite exclue)
- [x] 03-04: Ajouter « Ce que l'outil suppose » et « Ce que l'outil ne fait pas » — hypothèses prouvées par balayage sur la surface publique, limites adossées au code — puis le contrôle de clôture de la page et l'empreinte de `.data/`

**Écarts mesurés et reformulations enregistrées** (décision prise pendant la planification, jamais une assertion rouge sur une implémentation correcte) : **ÉCR-1** — la « table de correspondance libellé tronqué → nom complet » n'a pas d'objet dans ce flux (aucune ligne rendue ne porte `…`, largeur maximale exactement `COLS`), la table porte donc sur **libellé technique → nom complet** et aucun contrôle n'asserte l'apparition d'une troncature ; **ÉCR-2** — « `Méthode`/`Score`/`Indice de recherche` sur la dernière page » est faux sur la fixture (page 2/3), l'assertion est donc **positionnelle** (après le dernier `Équipement :`, avant `Greedy: `) et la page dit « en fin de résultat » ; **ÉCR-3** — les heuristiques de classe existent : elles sont présentées comme des préférences de style de jeu, ajustables après calcul ; **ÉCR-4** — l'écran affiche `base+parcho` alors que le parcours suppose zéro parchemin : une phrase lève l'ambiguïté ; **ÉCR-5** — la 21ᵉ sauvegarde évince la plus ancienne en silence : la page dit « les plus anciennes sont remplacées », jamais « 20 maximum » tout court.
...
- [x] 04-01: Trancher la source unique et rédiger `docs/wizard-avance.md` (9 étapes, slots, filtres, options, formats d'édition, syntaxe d'items) et son entrée d'index
...
- [x] 04-04: Ajouter le contrôle des renvois obsolètes, observé rouge sur l'état antérieur puis vert après correction

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 04-03: Réduire `GUIDE_WIZARD.md` à un aiguillage corrigé et rediriger `README.md`
...
**Plans**: 4/4 plans executed (05-04 : fermeture du critère 1, plan de fermeture de gap)
**UI hint**: no
...
- [x] 05-04: Fermer le critère 1 — dire les quatre conditions réelles du déclencheur (fenêtre écoulée, base vide, aucun dernier contrôle, `--force-sync`) dans `docs/base-locale.md`, borner l'affirmation de la base absente, et ajouter le contrôle qui **mesure** ses conditions sur `dofus_stuff/sync.py` (trois morsures exigées)
